 ...
 */
export const normalizeArray = (data: Float32Array, m: number, downsamplePeaks: boolean = false, memoize: boolean = false) => {
  let cache, mKey, dKey
  if (memoize) {
    mKey = m.toString()
    ...
    if (cache[mKey][dKey]) return cache[mKey][dKey]
  }
  const n = data.length
  ...
  if (m <= n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (downsamplePeaks) result[index] = Math.max(result[index], Math.abs(data[i]))
      else result[index] += Math.abs(data[i])
      count[index]++
    }
    if (!downsamplePeaks) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      const t = index - low
      if (high >= n) result[i] = data[n - 1]
      else result[i] = data[low] * (1 - t) + data[high] * t
    }
  }
  if (memoize) cache[mKey as string][dKey as string] = result
  return result
}
