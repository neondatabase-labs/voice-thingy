...
import { Coordinates, RealtimeEvent } from '@/lib/types'
          ...
          <div>
            ...
            <div ref={eventsScrollRef}>
              ...
              {realtimeEvents.map((realtimeEvent, i) => {
                ...
                if (event.type === 'input_audio_buffer.append') event.audio = `[trimmed: ${event.audio.length} bytes]`
                else if (event.type === 'response.audio.delta') event.delta = `[trimmed: ${event.delta.length} bytes]`
                return (
                  <div key={event.event_id}>
                    ...
                    <div>
                      ...
                      >
                        <div className={`${event.type === 'error' ? 'error' : realtimeEvent.source}`}>
                      ...
                      {!!expandedEvents[event.event_id] && <div>{JSON.stringify(event, null, 2)}</div>}
                    </div>
                  </div>
                )
              })}
